...
export const getServerSideProps = async (context) => {
  ...
  if (!slug?.includes("customize") && slug?.length <= 2) {
    ...
    if (productType !== "PRODUCT") {
      ...
      console.log('productFilterPromise', productFilterPromise, 'productsDataPromise', productsDataPromise, 'productLayoutPromise', productLayoutPromise);
      const [productLayout, productsData, productFilter] = await Promise.all([
        productLayoutPromise,
        productsDataPromise,
        productFilterPromise,
      ]);
console.log('11productFilterPromise', productFilterPromise, '22productsDataPromise', productsDataPromise, '33productLayoutPromise', productLayoutPromise);
    ...
    return (
      <React.Fragment>
        {productDetails?.LISTING && (
          <PdpShema
            listings={productDetails?.LISTING}
            router={router}
            seoData={layout.SEO}
          />
        )}
